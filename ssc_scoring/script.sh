...
idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_$idx.err 1>${slurm_dir}/slurm-${job_id}_$idx.out --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname=$(hostname) --epochs=1000 --sys=1 --sampler=0 --pretrained=1 --sys_pro_in_0=0.5 --sys_ratio=0.0 --mode='train' --fold=1 --gen_gg_as_retp=1 --remark="sampler+sys,sys_ratio=0.0, 16 patients in test dataset, including pat_070" &
#idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_$idx.err 1>${slurm_dir}/slurm-${job_id}_$idx.out --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname=$(hostname) --epochs=1000 --sys=1 --sampler=1 --pretrained=1 --sys_pro_in_0=0.0 --sys_ratio=0.25 --mode='train' --fold=1 --gen_gg_as_retp=1 --remark="sampler+sys+pretraining,sys_ratio=0.25, new data split, 16 patients in test dataset" &
